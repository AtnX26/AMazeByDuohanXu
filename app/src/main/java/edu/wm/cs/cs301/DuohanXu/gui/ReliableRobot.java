...
public class ReliableRobot implements Robot{
	 ...
	 * @param controller for the robot to set up with
	 * @throws exception while the controller is not correctly set up
	 * (for example, a null value)
	 */
	@Override
	public void setController(Control controller) {
		
		if (controller == null || controller.getMaze() == null||!(controller.currentState instanceof StatePlaying) ){
	 ...
	 * @param a float representing the battery level.
	 ...
	 * @param an enum that decides the type of turning the robot will execute.
	 */
	@Override
	public void rotate(Turn turn) {
		...
		switch(turn) {
			case LEFT:
				control.currentState.handleUserInput(UserInput.LEFT, 0);
				setBatteryLevel(getBatteryLevel()-rotatecost);
				break;
			case RIGHT:
				control.currentState.handleUserInput(UserInput.RIGHT, 0);
				setBatteryLevel(getBatteryLevel()-rotatecost);
				break;
			case AROUND:
				control.currentState.handleUserInput(UserInput.RIGHT, 0);
	 ...
	 * of battery. 
	 * @param and integer for distance to cover
	...
	public void move(int distance) throws IllegalArgumentException{
		...
		while (distance>0) {
			...
			if (getBatteryLevel() >= movecost) {
				control.currentState.handleUserInput(UserInput.UP, 0);
	...
	public void jump() {
		...
		control.currentState.handleUserInput(UserInput.JUMP, 0);
